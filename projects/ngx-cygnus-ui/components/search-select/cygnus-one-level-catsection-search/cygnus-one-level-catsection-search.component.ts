...
@Component({
  selector: 'cygnus-one-level-catsection-search',
  imports: [
    ReactiveFormsModule,
    CommonModule,
  ...
  templateUrl: './cygnus-one-level-catsection-search.component.html',
})
export class CygnusOneLevelCatSectionSearchComponent implements OnInit {
  TW_CLASS = TW_CLASS;
  ...
  ngOnInit(): void {
    ...
    this.selectSearchId.set(`cg-one-level-catsection-search-${++CygnusOneLevelCatSectionSearchComponent.idCounter}`);
    this.ulShowSecOptId.set(`cg-one-level-catsection-search-ul-sec-${++CygnusOneLevelCatSectionSearchComponent.idCounter}`);
